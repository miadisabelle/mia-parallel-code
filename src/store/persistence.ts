 ...
 * agents vs. saved defs), and passes collapsed=true for collapsed entries.
 */
function toPersistedTask(task: Task, agentDefs: AgentDef[], collapsed?: boolean): PersistedTask {
  return {
    id: task.id,
    name: task.name,
...
export async function saveState(): Promise<void> {
  ...
  for (const taskId of store.taskOrder) {
    ...
    persisted.tasks[taskId] = toPersistedTask(task, agentDefs);
  }
  ...
  for (const taskId of store.collapsedTaskOrder) {
    ...
    persisted.tasks[taskId] = toPersistedTask(task, agentDefs, true);
  }
  ...
  setStore(
    produce((s) => {
      ...
      if (s.activeTaskId && s.tasks[s.activeTaskId]) {
        ...
        s.activeAgentId = effectiveAgentId(task);
      }
    }),
  );
